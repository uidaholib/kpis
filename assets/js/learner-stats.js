...
class LearnerStats {
    ...
    async processData(csvText) {
      ...
      const getStatsForPeriod = (startDate, endDate) => {
        ...
        return {
          ...
          workshopSessions: workshops.reduce((sum, row) => sum + (row.number_of_sessions || 1), 0),
          workshopAttendees: workshops.reduce((sum, row) => sum + ((row.count || 0) * (row.number_of_sessions || 1)), 0),
          
          allUnique: (workshops.reduce((sum, row) => sum + (row.count || 0), 0)) + (courseBased.reduce((sum, row) => sum + (row.count || 0), 0)),
    ...
    render() {
      ...
      const rows = [
        ...
        { label: 'Total Unique Attendees', key: 'allUnique' },
